...
<div class="card p-3 mb-3"><h2 class="h5">Pedidos</h2><div class="table-responsive"><table class="table table-sm align-middle"><thead><tr><th>ID</th><th>Utilizador</th><th>Tema</th><th>Status</th><th>SLA</th><th>Prioridade</th><th>Tipo</th><th>Preço</th><th>Template aplicado (última versão)</th><th>Ações</th></tr></thead><tbody><?php foreach (($orders ?? []) as $o): ?><tr><td>#<?= (int) $o['id'] ?></td><td><?= htmlspecialchars((string) ($o['user_email'] ?? '-')) ?></td><td><?= htmlspecialchars((string) ($o['title_or_theme'] ?? '-')) ?></td><td><?= $badge((string) ($o['status'] ?? 'draft')) ?><?php $compliance = $o['latest_compliance_validation'] ?? null; if (is_array($compliance)): ?><?php $critical = (int) ($compliance['critical_count'] ?? 0); ?><?php if ($critical > 0): ?><div class="border rounded p-2 bg-light mt-1"><div class="small fw-semibold text-danger">Bloqueio de compliance: <?= $critical ?> crítica(s)</div><?php $issues = json_decode((string) ($compliance['non_conformities_json'] ?? '[]'), true); if (is_array($issues)): ?><ul class="small mb-0 mt-1"><?php foreach ($issues as $issue): ?><?php if ((string) ($issue['severity'] ?? '') !== 'critical') { continue; } ?><li><?= htmlspecialchars((string) ($issue['message'] ?? '-')) ?></li><?php endforeach; ?></ul><?php endif; ?><div class="small text-muted mt-1">Ação recomendada: regenerar/corrigir.</div></div><?php endif; ?><?php endif; ?></td><td><?= htmlspecialchars((string) ($o['sla_state'] ?? '-')) ?></td><td><?= htmlspecialchars((string) ($o['admin_priority'] ?? 'normal')) ?></td><td><?= htmlspecialchars((string) ($o['work_type_name'] ?? '-')) ?></td><td><?= $formatMoney($o['final_price'] ?? 0) ?></td><td><?php $tpl = $o['latest_template_application'] ?? null; if (is_array($tpl)): ?><div class="small"><strong><?= htmlspecialchars((string) ($tpl['template_file'] ?? '—')) ?></strong></div><div class="small text-muted">modo: <?= htmlspecialchars((string) ($tpl['mode'] ?? '—')) ?></div><?php else: ?><span class="text-muted small">Sem versão gerada</span><?php endif; ?></td><td>
...
<form method="post" action="/admin/orders/<?= (int)$o['id'] ?>/reopen-review" class="d-inline">
<input type="hidden" name="_csrf" value="<?= htmlspecialchars((string)($csrfToken ?? '')) ?>"><input type="hidden" name="confirm" value="1"><input type="text" name="reason" required placeholder="Motivo correção" class="form-control form-control-sm d-inline" style="width:130px"><button class="btn btn-sm btn-outline-primary">Regenerar/Corrigir</button></form>
