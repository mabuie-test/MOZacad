...
final class GenerateOrderDocumentJob
{
    ...
    {
        return (new AcademicFallbackPolicyService())->ensureSubstantiveDevelopment(
            $sections,
            $briefing,
            $referenceStyle,
            fn (array $currentSections): bool => $this->isDevelopmentSufficient($currentSections),
            fn (array $currentBriefing, string $currentReferenceStyle): array => $this->buildDefaultAcademicReferences($currentBriefing, $currentReferenceStyle),
            fn (string $line): string => $this->toInlineCitation($line),
            fn (array $section): string => $this->classifySectionKey($section)
        );
    }
    ...
    private function buildGenericAbstractReinforcement(array $briefing): string
    {
        ...
        return "Este estudo examina {$theme} tomando {$problem} como eixo de problematização e não apenas como descrição temática. O objectivo geral — {$objective} — orienta a articulação entre debate teórico, leitura histórica e interpretação das mediações institucionais que condicionam o fenómeno. Metodologicamente, mobiliza-se {$methodology}, com triangulação entre literatura especializada, fontes normativas e produção crítica do campo. A análise evidencia que permanências estruturais convivem com deslocamentos políticos e socioculturais, produzindo efeitos desiguais sobre actores e territórios. Em síntese, os achados sustentam a necessidade de interpretações multicausais e historicamente situadas, capazes de relacionar evidência empírica, categorias analíticas e implicações públicas. Palavras-chave: {$keywords}.";
    }
    ...
    private function buildGenericIntroductionReinforcement(array $briefing): string
    {
        ...
        $problem = trim((string) ($briefing['problem'] ?? 'o problema definido no briefing'));
        $objective = trim((string) ($briefing['generalObjective'] ?? 'o objectivo geral indicado'));
        return "O presente estudo situa {$theme} no interior de processos históricos de longa duração e de disputas políticas que moldaram agendas, instituições e desigualdades no campo analisado. Em vez de uma leitura linear, assume-se {$problem} como problema científico que exige explicação das continuidades, das rupturas e das relações de poder implicadas na produção do fenómeno. A pertinência académica decorre da necessidade de combinar historiografia, teoria social e análise documental para superar abordagens descontextualizadas. Nesse quadro, {$objective} funciona como princípio de recorte e de coerência argumentativa: cada secção do trabalho foi estruturada para relacionar contexto, autores e evidências, convertendo descrição em interpretação crítica sustentada.";
    }
    ...
    private function buildGenericConclusionReinforcement(array $briefing): string
    {
        ...
        return "À luz do problema {$problem}, a conclusão confirma que os resultados não podem ser explicados por causalidades simples: eles emergem da convergência entre heranças históricas, escolhas institucionais e disputas sociopolíticas contemporâneas. Retomando {$objective}, o percurso analítico mostrou coerência entre enquadramento teórico, estratégia metodológica e evidências discutidas, permitindo sustentar inferências com maior densidade crítica. Os principais achados apontam para a necessidade de rever categorias interpretativas naturalizadas no debate e de reposicionar o fenómeno em chave histórica e relacional. Reconhecem-se, contudo, limites ligados ao corpus e à variabilidade contextual; por isso, recomenda-se agenda futura com estudos comparativos, ampliação empírica e avaliação de efeitos de políticas, de modo a aprofundar a robustez explicativa aqui alcançada.";
    }
