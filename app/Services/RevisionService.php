...
final class RevisionService
{
    ...
    public function request(int $orderId, int $userId, string $reason): int
    {
        ...
        return $this->revisions->create($orderId, $userId, $reason, 'requested');
    }

    public function markReturnedForRevision(int $orderId, ?string $reviewerComment = null): void
    {
        ...
        $this->revisions->updateStatus((int) $latest['id'], 'returned', $reviewerComment);
    }

    public function markApproved(int $orderId, ?string $reviewerComment = null): void
    {
        $latest = $this->revisions->findLatestByOrderId($orderId);
        if ($latest === null) {
            return;
        ...
        $this->revisions->updateStatus((int) $latest['id'], 'approved', $reviewerComment);
    }
}
