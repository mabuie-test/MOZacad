...
final class InvoiceRepository extends BaseRepository
{
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM invoices WHERE order_id = :order_id AND status IN ('issued','pending') ORDER BY id DESC LIMIT 1 FOR UPDATE");
        $stmt->execute(['order_id' => $orderId]);
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM invoices WHERE order_id = :order_id ORDER BY id DESC LIMIT 1');
        $stmt->execute(['order_id' => $orderId]);
